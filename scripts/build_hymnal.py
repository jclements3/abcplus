...
for i, chunk in enumerate(tunes_raw):
    ...
    bpm = {'3/4':3, '6/8':6, '2/4':2, '2/2':2, '3/8':3}.get(meter, 4)

    # Parse soprano melody
    soprano = []
    for line in chunk.split('\n'):
        if '[V: S1V1]' in line:
            soprano.extend(parse_voice(line))
    if not soprano: continue

    # Group into measures
    measures = []
    beat = 0; m_notes = []; m_midis = []; m_acc = False
    for midi, dur, acc, abc_note in soprano:
        m_notes.append(abc_note)
        m_midis.append(midi)
        if acc: m_acc = True
        beat += dur
        if beat >= bpm - 0.01:
            measures.append({'notes': m_notes, 'midis': m_midis, 'has_acc': m_acc, 'beats': bpm})
            m_notes = []; m_midis = []; m_acc = False; beat = 0
    if m_notes:
        measures.append({'notes': m_notes, 'midis': m_midis, 'has_acc': m_acc, 'beats': bpm})
